...
function FirewallSettings {
    ...
    Write-Host "Current Firewall Settings"
...
function ViewConfiguration {
    if ( $FirstOption -eq "View" ){
       ...
       Write-Host "Hardens Chrome web browser settings(Disables screen, audio, and video capture, enables always opening PDFs externally, 
       blocks external extensions, enables DNS over HTTPS, etc)"
       Write-Host "------"
       ...
       HTTP printing, etc enables data execution prevention, protection mode, etc)"
...
function BackupRegistry {
    reg export HKCR C:\ASH\WINREG-Backups\HKCR.REG
    reg export HKCU C:\ASH\WINREG-Backups\HKCU.REG
    reg export HKLM C:\ASH\WINREG-Backups\HKLM.REG
    reg export HKU C:\ASH\WINREG-Backups\HKU.REG
    reg export HKCC C:\ASH\WINREG-Backups\HKCC.REG
...
function Main {
    ...
    Write-Host "---------------------------------------"
    Write-Host ""
    $MainOption = Read-Host -Prompt "Type 'ASH' to start system hardening, type 'View' to view the changes this program will make, 
    type 'RestoreReg' to restore Windows Registry from backup, or type 'CleanDisk' to delete temporary and junk files"
    if ( $MainOption -eq "ASH" ){
        $Mode_Choice = Read-Host -Prompt "Choose hardening mode: Automated or Manual"
    }
    elseif ( $MainOption -eq "View" ){
        ViewConfiguration 
    }
    elseif ( $MainOption -eq "RestoreReg" ){
        reg import C:\ASH\WINREG-Backups\HKCR.REG
        reg import C:\ASH\WINREG-Backups\HKCU.REG
        reg import C:\ASH\WINREG-Backups\HKLM.REG
        reg import C:\ASH\WINREG-Backups\HKU.REG
        reg import C:\ASH\WINREG-Backups\HKCC.REG
        Write-Host "Windows Registry has been restored from backup"
    }
    elseif ( $MainOption -eq "CleanDisk" ){
        DiskCleanup
    }
